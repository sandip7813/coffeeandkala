    ...
    @if (count($category['articles']))
        <section class="fc-art-catalogue" aria-label="Articles in {{ $category['name'] }}">
            @foreach ($category['articles'] as $article)
                <article class="fc-art-entry features-reveal features-reveal--up"
                    style="--portal-delay: {{ $loop->index * 0.06 }}s">
                    <a href="{{ $article['href'] }}" class="fc-art-entry-link">
                        <span
                            class="fc-art-no">{{ $category['number'] }}.{{ str_pad((string) $loop->iteration, 2, '0', STR_PAD_LEFT) }}</span>
                        <div class="fc-art-thumb">
                            <img src="{{ $article['image'] }}" alt="" loading="lazy" width="160"
                                height="160" decoding="async">
                        </div>
